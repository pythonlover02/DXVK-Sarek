...
namespace dxvk {
  ...
  uint32_t SpirvModule::opSinCos(
    ...
    uint32_t xNorm  = opFMul(floatType, opFAbs(floatType, x),
                             constf32(4.0f / pi));
    ...
    uint32_t mirror = opINotEqual(boolType,
      opBitwiseAnd(uintType, xInt, constu32(1u)), constu32(0u));
    xFract = opSelect(floatType, mirror,
      opFSub(floatType, constf32(1.0f), xFract), xFract);
    ...
    uint32_t taylor = opFMul(floatType, xFract_6,
      constf32(static_cast<float>(-sincosTaylorFactor(7))));
    decorate(taylor, spv::DecorationNoContraction);

    taylor = opFFma(floatType, xFract_4,
      constf32(static_cast<float>(sincosTaylorFactor(5))), taylor);
    decorate(taylor, spv::DecorationNoContraction);

    taylor = opFFma(floatType, xFract_2,
      constf32(static_cast<float>(-sincosTaylorFactor(3))), taylor);
    decorate(taylor, spv::DecorationNoContraction);

    taylor = opFAdd(floatType,
      constf32(static_cast<float>(sincosTaylorFactor(1))), taylor);
    decorate(taylor, spv::DecorationNoContraction);

    taylor = opFMul(floatType, taylor, xFract);
    decorate(taylor, spv::DecorationNoContraction);

    uint32_t coFunc = opSqrt(floatType,
      opFSub(floatType, constf32(1.0f),
             opFMul(floatType, taylor, taylor)));

    uint32_t funcIsSin = opIEqual(boolType,
      opBitwiseAnd(uintType,
        opIAdd(uintType, xInt, constu32(1u)), constu32(2u)),
      constu32(0u));

    uint32_t sin = opSelect(floatType, funcIsSin, taylor, coFunc);
    uint32_t cos = opSelect(floatType, funcIsSin, coFunc, taylor);

    uint32_t sinNeg = opINotEqual(boolType,
      opBitwiseAnd(uintType, xInt, constu32(4u)), constu32(0u));
    sinNeg = opLogicalNotEqual(boolType, sinNeg,
      opFOrdLessThan(boolType, x, constf32(0.0f)));

    uint32_t cosNeg = opINotEqual(boolType,
      opBitwiseAnd(uintType,
        opIAdd(uintType, xInt, constu32(2u)), constu32(4u)),
      constu32(0u));

    sin = opSelect(floatType, sinNeg,
      opFNegate(floatType, sin), sin);
    cos = opSelect(floatType, cosNeg,
      opFNegate(floatType, cos), cos);
